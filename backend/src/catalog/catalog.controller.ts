...
export class CatalogController {
  ...
  async catalog() {
    ...
    return {
      ...
      procurement: {
        label: '采购合同/国内备货',
        n5: '销售合同与采购合同分开签订。采购合同/PO 金额固定人民币。公司惯例先销售后采购：保存或推进采购须关联一笔已达 N3 且已签的销售/出口合同。登记国内供应商、采购合同/PO、供应商实际交付日期与实际交付日期、付款方式（一次性付清或分期支付）。一次性付清不要求、也不登记已付货款与付款日期。分期每一期须填约定付款时间、付款比例、金额，并可登记本期已付与付款日。供应商须过制裁/不可靠实体筛查；供应商实际交付日期或实际交付日期任一不得晚于关联销售合同交货期，否则须结构化延期并保留客户同意证据',
      ...
      sales: {
        ...
        n3: '买方与收货人在本节点填写并筛查，须通过后才能推进。运输术语仅 FOB / CIF，与结算方式（前 T/T / 后 T/T）独立。后 T/T 填写到达目的港后付款天数。销售合同金额可选人民币或美元。交货方式为自有仓或港口直出；直出填货物仓储地点、批次号。货物名称与规格默认可从报价带入，与采购合同各自保存。销售合同填写装运港与装运期限（与装运页 CIF 装运港口、装运日期分开）。中信保保单须上传文件并写入证据链。CIF 装运（含货物状态）在装运页办理。前 T/T 登记收汇凭证、收汇金额与比例。',
        ...
        n6n7:
          'N6 须上传商业发票与箱单并完成内部审批。装运规则跟随所选运输术语：FOB/EXW/FAS/FCA 买方安排运输（可无提单）；CIF/CFR 等卖方出单（须正本或电放）。不再要求客户书面指示或订舱编号。N7 六份必填：销售合同、商业发票、箱单、采购合同、发票、报关单；另有原产地证，运输术语为 FOB 时可不传，CIF 等非 FOB 必填，缺则 409。过闸后进入收汇 N9，不再经过报关放行。T/T 不是 Incoterm。无有效运输术语时明确回退为 FOB。存在未生效变更单时 N6/N7/N9 硬拦截，须先生效变更。',
      },
      taxFinance: {
        label: '出口退税与融资性贸易审查',

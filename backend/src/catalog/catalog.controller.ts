  ...
  async catalog() {
    ...
    return {
      ...
      sales: {
        ...
        grouping:
          '列表按未出运 / 已出运 / 已完成分组。已出运：装运日期已填，或 N6 已通过/已过装运节点，或已有提单号，或 FOB 等无提单路径已登记。已完成：已出运且客户已提货且已收汇且未收汇金额为 0。已出运列不含已完成。',
        n3: '打开即填写销售合同。CIF/CIP 显示装运港口、装运日期、预计到达日期与到达港口；客户是否提货与收汇适用于全部术语。FOB 等装运/提单仍在 N6（无提单路径不变）。任意术语均可登记约定客户付款日期、是否收汇、收汇金额；未收汇金额按合同总额−收汇金额自动计算。',
      },

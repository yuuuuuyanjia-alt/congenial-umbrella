...
| N3 | 销售合同/订单确认 | **销售/出口合同**与采购合同分开签订。**买方、收货人在本页填写**（可改，不从已取消的询盘页只读带入），并做 OFAC / UN / EU / UK / 中国不可靠实体模拟筛查；高置信命中硬拦截，须通过后才能推进。付款人不是必填。金额可选 **CNY / USD**（占用不换汇，仅美元计入美元占用；限额仍登记美元）。**运输术语**仅 FOB / CIF，与**结算方式**（前 T/T / 后 T/T）独立。数量单位吨/千克。**货物名称 + 规格**默认可从报价带入，与采购合同各自保存。交货方式自有仓 / 港口直出；直出填仓储地点、批次号。前 T/T 登记收汇凭证、收汇金额与比例。**CIF 装运在 N6**。销售合同列表按 **未出运 / 已出运 / 已完成** 分组 |
| N4 | 变更管理 | 交货期 / 数量 / 收货人 / 付款条件变更须出变更单（含 diff）；客户确认 + 内部确认；敏感变更须审批；旧版 `SUPERSEDED`；当事方/付款/收货人变更会重跑关联节点闸门；**进入变更时须再次确认中信保并按变更后金额重算占用** |
| N5 | 采购合同/国内备货 | **先销售后采购**：须关联一笔已达 N3 且已签的销售合同案件（客户、合同号、金额、状态），否则不得保存或推进；采购合同/PO 金额 **固定人民币**；**货物名称 + 规格独立可改**，不回写销售合同，也不从销售合同只读锁定。**不展示收货人**。登记国内供应商、采购合同/PO、**供应商实际交付日期**与可选**实际交付日期**；付款方式选择 **一次性付清** 或 **分期支付**（每期须填约定付款时间、付款比例、金额）；供应商过 OFAC/UN/EU/UK 与中国不可靠实体模拟筛查（高置信硬拦截、中置信审核队列、低置信软提示）；供应商实际交付日期或实际交付日期**任一**晚于**关联销售合同交货期**，须登记**结构化延期触发条件**；表单展示对照用的销售合同交货期；客户同意延期须有可追溯证据编号；无同意则中风险不得推进 |
...
进入装运前先打开 **选择出运批次**：点选已有批次，或填写批次号、数量、金额后新建。选定后进入该批次的 N6，再用本页「进入下一步」到 N7、N9。销售合同列表在下一步为装运时，「进入下一步」进入该选择页，不会打开没有 `batchId` 的空白装运页。

验证：销售合同详情没有「出运批次」区块。对已到装运的案件点列表「进入下一步」，应先看到批次选择/新建，选定后再进入该批 N6。再新建一批并分别进入两批的 N6。推进其中一批不应要求另一批先完成。
...
| --- | --- | --- | --- | --- |
| 业务 | `SALES` | 业务 | 客户 / 销售 / 采购录入，N2–N9 过闸。工作台领取/放行/驳回隐藏且接口拒绝 | **销售合同、采购合同**、供应商、客户（无工作台入口） |
| 风控 | `RISK` | 风控 | 工作台领取/放行/驳回与筛查处置。可查看合同；未扩大金额改写 | 工作台优先，其后销售合同/采购合同/客户/供应商 |
| 主管 | `MANAGER` | 主管 | 本轮只读（评估、占用、列表）。不可审批、推进、改合同或新建 | 客户、销售合同、采购合同优先（无工作台入口） |
...
| 入口 | 列表口径 | 填写路径 |
| --- | --- | --- |
| **销售合同管理** | 已到达销售合同/订单确认（N3）或已有销售合同的出口案件。仍在报价（N2）的案件不出现；`DEMO-BLOCK` 已到 N3（买方筛查硬拦截）会出现在未出运，但不能推进。列表按 **未出运 / 已出运 / 已完成** 分组（H5 三段入口 + 列表筛选）。**已出运**：CIF 装运日期、FOB 到达口岸时间、电汇结算下装运日期，或 N6/提单/无提单路径。**已完成**：已出运且已提货且 **N9 已回款**（水单/到账且未收汇为 0；与占用释放同一口径）。打开后进入 N3；**本案已离开 N3** 时提供短 CTA「进入下一步」。下一步为装运时，该按钮先进入出运批次选择页，再打开所选批次的 N6 | 打开即进入 N3。买方、收货人可改并在本页筛查。货物名称/规格可改，不锁定采购合同。**运输术语** FOB / CIF 与**结算方式** 前 T/T / 后 T/T 独立；金额可选 CNY / USD。CIF 装运在 N6。**新建销售合同**：短表（标题 + 金额）调用 `POST /api/cases`（演示默认品名/目的地/25,000 USD），**打开报价（N2）** |
| **采购合同管理** | 已到达国内采购/备货（N5）或已登记采购合同/PO 的案件。打开后进入 N5，列表与表单默认只暴露采购内容。N6–N9 仍挂在同一出口案件上，不迁入采购 PO。已离开 N5 或保存/过闸后，按钮改为「去办装运（出口案）」等（按当前出口节点：装运/单证/收汇）。去办装运时先进入出运批次选择，再打开该批次 N6；不把装运后续写成采购合同的「下一节点」。销售合同管理仍用「进入下一节点」；当前步骤为装运时先进入出运批次选择，而不是无批次的空白 N6 | 打开即进入既有 **N5 采购合同** 表单；**关联销售合同**为可任选的已签销售合同列表（`GET /api/cases/:id/nodes/N5/sales-options`），不锁定本案。保存/推进须已选（`salesCaseId`，闸门 `N5_SALES_LINK` / `N5_SALES_NOT_SIGNED`）。货款 **一次性付清** 或 **分期支付**（每期约定付款时间、付款比例、金额）。**新建采购合同**：不调用 `POST /cases`；须点选一笔**已签订的销售合同**，打开**该案** N5（已有 PO 则编辑） |

...
| 入口 | 列表口径 | 填写路径 |
| --- | --- | --- |
| **销售合同管理** | 已到达销售合同/订单确认（N3）或已有销售合同的出口案件。询盘未通过（如 `DEMO-BLOCK`）不出现。列表按 **未出运 / 已出运 / 已完成** 分组（H5 三段入口 + 列表筛选）。**已出运**：CIF 装运日期、FOB 到达口岸时间、电汇结算下装运日期，或 N6/提单/无提单路径。**已完成**：已出运且已提货且 **N9 已回款**（水单/到账且未收汇为 0；与占用释放同一口径）。勾选 N3「是否收汇」不能假装完成。演示：`DEMO-NORD-WIP`（CIF + 后 T/T）/`DEMO-HELIOS-WIP`（FOB + 前 T/T）未出运，`DEMO-NORD-OPEN` / `DEMO-FOB` / `DEMO-HELIOS-DONE` / `DEMO-NORD-LATE` 已出运，`DEMO-PASS` 已完成。打开后进入 N3；**本案已离开 N3** 时列表与表单提供「进入下一节点」，直达当前九节点（N3 无变更单则下一步为 N5）。过闸成功后自动打开下一节点 | 打开即进入 N3。**运输术语** FOB / CIF 与**结算方式** 前 T/T / 后 T/T 独立勾选，字段均可填；收汇金额只读。**新建销售合同**：短表（标题 + 金额）调用 `POST /api/cases`（演示默认品名/目的地/25,000 USD，买方三当事方同名预填），然后打开 N3。`create` 返回案件仍停在 **N1**，不能跳过闸门：确认签订须先完成询盘 KYC 与报价；保存合同仍须登记中信保限额。表单会提示并提供「去办询盘 / 客户 KYC」 |
| **采购合同管理** | 已到达国内采购/备货（N5）或已登记采购合同/PO 的案件。打开后进入 N5，列表与表单默认只暴露采购内容。N6–N9 仍挂在同一出口案件上，不迁入采购 PO。已离开 N5 或保存/过闸后，按钮改为「去办装运（出口案）」等（按当前出口节点：装运/单证/报关/收汇），跳到出口案对应页，不把装运后续写成采购合同的「下一节点」。销售合同管理仍用「进入下一节点」直达本案当前九节点 | 打开即进入既有 **N5 采购合同** 表单；**关联销售合同**为可任选的已签销售合同列表（`GET /api/cases/:id/nodes/N5/sales-options`），不锁定本案。保存/推进须已选（`salesCaseId`，闸门 `N5_SALES_LINK` / `N5_SALES_NOT_SIGNED`）。货款 **一次性付清** 或 **分期支付**（每期约定付款时间、付款比例、金额）。**新建采购合同**：同样 `POST /api/cases` 后打开 N5；**不会**自动关联销售合同，须在本页任选一笔已签合同 |
...
- `POST /api/cases` 新建出口案（从 N1 起；可选 `buyerName` / `buyerCountry` 预填买方/付款人/收货人）。业务岗「新建销售合同」走此接口

...
| 节点 | 名称 | 要点 |
| --- | --- | --- |
| N1 | 询盘/客户KYC | 买方 / 收货人；**货物名称 + 规格**（带入报价与销售合同，不默认「机械」）；模拟筛查 OFAC、UN、EU、UK、中国不可靠实体清单；KYC 报告 + 风险评分；高置信命中硬拦截。付款人不再作为询盘必填项 |
| N2 | 报价环节 | 货物名称 + 规格（可改，带入 N3）；所含项目（海运费 / 陆运费 / 港杂多选）、有效期、单价（吨或千克）；「价格待定/费用另议」禁止推进；相对成本底线/历史价异常偏离软提示；报价版本 + 字段快照入审计 |
| N3 | 销售合同/订单确认 | **销售/出口合同**与采购合同分开签订。相对方只读带自 N1。金额可选 **CNY / USD**（占用不换汇，仅美元计入美元占用；限额仍登记美元）。**运输术语**仅 FOB / CIF，与**结算方式**（前 T/T / 后 T/T）独立。数量单位吨/千克。交货方式自有仓 / 港口直出；直出填仓储地点、批次号、电子底账编号。前 T/T 登记收汇凭证、收汇金额与比例。**CIF 装运在 N6**。销售合同列表按 **未出运 / 已出运 / 已完成** 分组 |
...
| 入口 | 列表口径 | 填写路径 |
| --- | --- | --- |
| **销售合同管理** | 已到达销售合同/订单确认（N3）或已有销售合同的出口案件。询盘未通过（如 `DEMO-BLOCK`）不出现。列表按 **未出运 / 已出运 / 已完成** 分组（H5 三段入口 + 列表筛选）。**已出运**：CIF 装运日期、FOB 到达口岸时间、电汇结算下装运日期，或 N6/提单/无提单路径。**已完成**：已出运且已提货且 **N9 已回款**（水单/到账且未收汇为 0；与占用释放同一口径）。打开后进入 N3；**本案已离开 N3** 时提供短 CTA「进入下一步」 | 打开即进入 N3。相对方只读；货物名称/规格可改。**运输术语** FOB / CIF 与**结算方式** 前 T/T / 后 T/T 独立；金额可选 CNY / USD。CIF 装运在 N6。**新建销售合同**：短表（标题 + 金额）调用 `POST /api/cases`（演示默认品名/目的地/25,000 USD），**打开询盘/客户 KYC（N1）** |
...
- `POST /api/cases/:id/nodes/N1/inquiry` 保存询盘货物名称 / 规格（贯通 N2/N3）
...
- **交货方式**：销售合同必选自有仓 / **港口直出**（不再选保税）。港口直出须填货物仓储地点、批次号、电子底账编号，否则 409。薄利+直出黄灯进工作台；像空转或购销/报关货描严重不符为红线硬拦截。退税申报前须 FT4 就绪清单。

...
> 制裁筛查为 **本地模拟黑名单 + 接口抽象**，不包含、也不需要真实 OFAC/UN 等 API Key。报关放行页已取消，不再模拟电子口岸同步。

## 范围

设计口径：用户可见流程从 **报价（N2）** 起，**2 → 3 → 4（按需）→ 5 → 6 → 7 → 9**。报关放行（原 N8）已取消，N7 过闸后进入收汇。不再单设询盘/客户 KYC 页。**销售合同（N3）与采购合同（N5）分开签订**；公司惯例 **先销售后采购**。N3 之后若无变更单则跳过 N4；一旦产生未生效变更，须完成客户/内部确认后才能进入国内采购/备货。进入 N5 时须**关联一笔已签订的销售合同案件**（至少已过 N3 且已签销售合同），否则不得保存或推进采购。公司无自有产线，签约后向**国内供应商**采购备货。

| 节点 | 名称 | 要点 |
| --- | --- | --- |
| N2 | 报价环节 | 货物名称 + 规格（可改，作为 N3 默认值）；所含项目（海运费 / 陆运费 / 港杂多选）、有效期、单价（吨或千克）；「价格待定/费用另议」禁止推进；相对成本底线/历史价异常偏离软提示；报价版本 + 字段快照入审计。新建销售合同先打开本页 |
| N3 | 销售合同/订单确认 | **销售/出口合同**与采购合同分开签订。**买方、收货人在本页填写**（可改，不从已取消的询盘页只读带入），并做 OFAC / UN / EU / UK / 中国不可靠实体模拟筛查；高置信命中硬拦截，须通过后才能推进。付款人不是必填。金额可选 **CNY / USD**（占用不换汇，仅美元计入美元占用；限额仍登记美元）。**运输术语**仅 FOB / CIF，与**结算方式**（前 T/T / 后 T/T）独立。后 T/T 的付款天数字段界面为 **到达目的港后付款天数**（仍写入 `ttDaysAfterShipment`）。数量单位吨/千克。**货物名称 + 规格**默认可从报价带入，与采购合同各自保存。交货方式自有仓 / 港口直出；直出填仓储地点、批次号。前 T/T 在本页只登记**约定比例与约定金额**，不上传收汇凭证。**CIF 装运在 N6**。销售合同列表按 **未出运 / 已出运 / 已完成** 分组 |
| N4 | 变更管理 | 交货期 / 数量 / 收货人 / 付款条件变更须出变更单（含 diff）；客户确认 + 内部确认；敏感变更须审批；旧版 `SUPERSEDED`；当事方/付款/收货人变更会重跑关联节点闸门；**进入变更时须再次确认中信保并按变更后金额重算占用** |
| N5 | 采购合同/国内备货 | **先销售后采购**：须关联一笔已达 N3 且已签的销售合同案件（客户、合同号、金额、状态），否则不得保存或推进；采购合同/PO 金额 **固定人民币**；**货物名称 + 规格独立可改**，不回写销售合同，也不从销售合同只读锁定。**不展示收货人**。登记国内供应商、采购合同/PO、**供应商实际交付日期**与可选**实际交付日期**；付款方式选择 **一次性付清** 或 **分期支付**。一次性付清只登记约定付款时间（及可选付款条件），**不登记已付货款、付款日期**。分期支付每一期须填约定付款时间、付款比例、金额，并仍可登记本期已付与付款日。供应商过 OFAC/UN/EU/UK 与中国不可靠实体模拟筛查（高置信硬拦截、中置信审核队列、低置信软提示）；供应商实际交付日期或实际交付日期**任一**晚于**关联销售合同交货期**，须登记**结构化延期触发条件**；表单展示对照用的销售合同交货期；客户同意延期须有可追溯证据编号；无同意则中风险不得推进 |
| N6 | 装运/提单指示 | **硬闸门**：上传商业发票 + 上传箱单（证据链）+ 内部审批。不再要求客户书面指示或订舱编号。CIF 填装运港口、装运日期、预计到港。装运规则跟随所选**运输术语**：CIF/CFR 等须 **正本或电放其一**；FOB/EXW/FAS/FCA 可走 **无提单**。T/T 是结算方式不是 Incoterm；无有效运输术语时**回退按 FOB**。过闸后本页「进入下一步」 |
| N7 | 单证一致性 | **硬闸门**：销售合同、商业发票、箱单、采购合同、发票、报关单六份必填；**原产地证**在运输术语不是 FOB 时必填（FOB 可不传）。**商业发票与发票分别必填**，缺则 **409**。不再核提单、装船通知或字段勾选，也不再核 HS/申报要素。过闸后本页「去收汇」进入**同一批次**的 **N9 收汇** |
| N9 | 收汇对账 | **硬闸门**：第三方关系证明（代付）+ 汇款附言 + 单证一致证明 + 放行审批。**水单/到账为已回款唯一账本**，销售列表「已完成」与占用「已回款」共用此口径。**按出运批次各记一笔**；客户合并付款时由业务员拆成各批次入账 |

N3 之后，一份销售合同可以有多笔 **出运批次**。每批独立批次号、数量/金额、提单路径（N6）、单证（N7）和收汇（N9），多批可以同时在途。买方、运输术语、币种、中信保限额仍在销售合同上。采购合同（N5）本轮不按批次拆 PO。中信保占用 = 各批次未收汇 + 尚未出运的合同余额（仍只把美元计入美元占用池）。已有案件迁移为默认批次 `1`，演示路径保持单批。销售合同详情与 N3 **不再**列出或新建批次。

首页工作入口包含 **出运管理**、**单证管理**、**收汇管理** 与 **费用管理**（业务、风控、主管都能看到；主管只读）。收汇（N9）仍按出运批次各记一笔，入口与单证一样先选合同再选已有批次。

- **出运管理**：先选销售合同，再选择或新建出运批次，进入该批次的装运/提单指示（N6）。选定后用本页「进入下一步」到 N7。装运硬闸门通过后，另有「去收汇」打开**同一批次**的 N9。
- **单证管理**：先选销售合同，再选择**已有**批次，进入该批次的单证一致性（N7）。没有批次时提示先去出运，不从单证新建空批次，也不打开没有 `batchId` 的空白单证页。单证硬闸门通过后，「去收汇」打开**同一批次**的 N9。
- **收汇管理**：先选销售合同。选定后即可填写**合同前收汇**（前 T/T 凭证、比例、金额；按合同保存，不绑定批次；还没有批次也可以填）。再选择**已有**批次，进入该批次的收汇对账（N9）。没有批次时提示先去出运，不新建批次，也不打开没有 `batchId` 的空白收汇页。直接打开 N9 且不带 `batchId` 时，落到与单证相同的批次选择页。N9 页头只读展示合同号、客户、批次、本批金额，以及 N3 的前/后 T/T 摘要（约定比例、约定金额、到达目的港后付款天数）。一批次一笔收汇；合并付款仍由业务员按批次拆开。闸门规则本轮不改。
- **费用管理**：先选销售合同，再登记该合同的费用。固定项为海运费、陆运费、港杂、保险，另可增删自定义费用（名称 + 金额）。全部选填，可以整单留空保存。数据在 `ContractFee`（`caseId` 唯一），金额币种固定 **CNY**，不跟随销售合同。传入其他币种会拒绝保存。不按出运批次，也不进入 N 节点闸门。报价「所含项目」仍只在 N2 多选，本页只把最新报价的所含项目当作提示，不回写报价。接口：`GET/POST /api/cases/:id/fees`。
- 销售合同列表「下一步」若是装运、单证或收汇，与上面同一路径（合同已选定，进入对应的批次选择）。收汇若该合同只有一个批次，则直接打开该批 N9（URL 带 `batchId`），不打开空白收汇页；没有批次或多个批次时进入批次选择。案件详情里点 N6/N7/N9 同样先选批次，不再跳进空白节点页。

验证：销售合同详情没有「出运批次」区块。首页能看到出运管理、单证管理、收汇管理。出运：选销售合同 → 选择或新建批次 → 该批 N6。单证：选销售合同 → 只选已有批次 → 该批 N7；没有批次时提示去出运。收汇：选销售合同 → 合同前收汇可填；只选已有批次 → 该批 N9；没有批次时提示去出运，仍可填合同前收汇。N3 不再有前 T/T 凭证上传。N6/N7 过闸后「去收汇」进入同一批次 N9。对已到装运的案件点列表「进入下一步」，应先看到批次选择/新建，选定后再进入该批 N6。下一步若是单证，则只列出已有批次。下一步若是收汇，唯一批次直接进入该批 N9。再新建一批并分别进入两批的 N6。推进其中一批不应要求另一批先完成。
...
一人一岗：登录后只渲染当前角色的菜单与按钮。顶部默认只读「当前：业务 / 风控 / 主管」，**不展示三选一切换**。后端按请求头强制校验，前端隐藏只是演示便利。演示/QA 可点首页底部「切换演示角色」，或连点版本号（或顶部当前角色）三次展开账号选择器。

切换后写入本地存储，之后请求带 `x-actor-id` 与 `X-Demo-Role`。亦可只传其一：有用户 id 时以该用户角色为准；否则按 `X-Demo-Role` 取该角色的种子用户。

| 账号 | `User.role` | 岗位 | 可写 | 首页入口顺序 |
| --- | --- | --- | --- | --- |
| 业务 | `SALES` | 业务 | 客户 / 销售 / 采购录入，N2–N9 过闸。工作台领取/放行/驳回隐藏且接口拒绝 | **销售合同、采购合同**，其后 **出运管理、单证管理、收汇管理、费用管理**，再供应商、客户（无工作台入口） |
| 风控 | `RISK` | 风控 | 工作台领取/放行/驳回与筛查处置。可查看合同；未扩大金额改写 | 工作台优先，其后销售合同/采购合同、出运管理/单证管理/收汇管理/费用管理、客户/供应商 |
| 主管 | `MANAGER` | 主管 | 本轮只读（评估、占用、列表）。不可审批、推进、改合同或新建 | 客户、销售合同、采购合同优先，其后出运管理/单证管理/收汇管理/费用管理（无工作台入口；收汇只读） |
...
覆盖：N3 买方筛查高置信硬拦截 / 低置信软提示、**无付款人亦可过闸**、未筛查不得推进；N2 模糊报价拒绝、所含项目/单价单位、成本底线偏离软提示；N3 缺条款拒绝、**中信保限额未登记不得签订合同 / 缺保单/占用超高风险拒绝、高风险工作台领取/放行/驳回、中风险软提示**、**CIF/CIP 装运节点**、**交货方式（不强制自有仓）与港口直出四问（FT1/FT2）**；N4 变更确认链、**变更后占用复核（高风险同样工作台审核）**、无变更跳过且不要求 N4 中信保、收货人被清空时重跑买方筛查；N5 **须关联已签销售合同**、供应商高置信硬拦截、采购交付延期无同意中风险、**购销匹配与薄利+直出黄灯（FT1）**、**货物名称/规格与销售合同分列**；**N6 商业发票+箱单上传、正本/电放二选一、FOB 无提单路径（不再要求书面指示或订舱编号）、缺上传拒绝**；**N7 六份上传缺任一份硬拦截（推进 409），商业发票与发票分开；CIF 等非 FOB 缺原产地证同样 409，FOB 可不传；N7 过闸进入 N9**；N9 缺证据拒绝推进；**直出货描严重不符红线（FT3，N6/N7）**；**采购货款一次性付清或分期支付（每期约定付款时间、付款比例、金额）与按期次逾期**；**先销售后采购：未签销售合同不得作为采购关联对象，已签合同可任选、不自动锁定本案**；**销售/采购合同列表按 N3 / N5 过滤，不混用采购 PO 作为销售合同条目**；**已回款单一事实源：列表已完成与占用释放均认 N9 水单/到账，勾选 N3 是否收汇不能假装完成**；**退税就绪清单 FT4（报关放行、N9 收汇、进项发票号、四流勾选）**；**新建销售合同从 N2 报价起，再到 N3；新建采购合同须点选已签销售合同后打开该案 N5**。
...
| 案件号 | 路径 | 说明 |
| --- | --- | --- |
| `DEMO-PASS` | 绿灯通过 | Nordlicht GmbH，清单未命中；国内供应商苏州精工机械筛查通过；CIF **正本提单**过 N6；报价 v1 废止 / v2 生效；中信保限额 150,000 USD；变更单 CO-001 数量 8→10 已确认生效并再次核对占用；**采购合同 `PO-BH-2026-011` 关联销售合同 DEMO-PASS**；采购交付未延期；CIF 单证含原产地证后进入收汇（不再经过报关放行页；库内原报关记录仍供退税清单读取）；**销售合同已填 CIF 装运港 Shanghai、装运日期、预计到港 Hamburg 与已提货**；**N9 水单已收汇全额**，销售列表 **已完成**，占用已释放 |
...
| `DEMO-BLOCK` | 硬拦截 | `Banned Trading LLC` 高置信命中模拟 OFAC。报价已过，停在 **N3**，买方筛查拒绝推进销售合同；**已到达 N3，会进入客户管理，但不得过闸** |
| `DEMO-GATE` | 闸门演示 | 已过 KYC / 报价 / **销售合同**（FOB，含中信保）/ **无变更故跳过 N4** / 国内采购备货（`PO-HV-2026-088` **关联 DEMO-GATE**），停在 N6，商业发票与箱单尚未上传 |
| `DEMO-FOB` | FOB 无提单 | Pacific Tools，FOB 买方订舱；N3 已填国内段到达口岸/港口时间（2026-12-08 10:00，完成国内交付）；N6 走「无提单」路径（内部审批，不要求书面指示或订舱编号），已过闸停在 N7。N7 须再上传销售合同、商业发票、箱单、采购合同、发票、报关单六份才能推进；FOB **可不传原产地证**。过闸后进入收汇（N9），不再经过报关放行。采购 `PO-PT-FOB-004` **关联销售合同 DEMO-FOB**。销售列表归入 **已出运** |
...
| `DEMO-PORT-YELLOW` | 港口直出黄灯 | Ostsee Tools，**港口直出**四问齐全（不强制自有仓）；销售 50,000 / 采购 49,200 USD 薄利 → **退税·融资性工作台**（领取 / 通过 / 驳回）。停在 N5。放行后可推进 |
| `DEMO-PORT-RED` | 港口直出红线 | Loopturn Trading 自答「像空转」，**红线硬拦截**；工作台第三页只读，不可通过 |
...
| `DEMO-NORD-OPEN` | 收汇未到期 | Nordlicht 在手已装运订单，已过单证进入收汇，约定到期 2026-12-31，尚无到账，**未到期**；采购 `PO-BH-2026-019` **关联 DEMO-NORD-OPEN** |
| `DEMO-NORD-WIP` | 未履行完毕 | Nordlicht **CIF + 后 T/T** 已签销售合同停在国内采购，尚未装运，未回款 18,000 USD；**待登记采购合同并关联本销售案件** |

## 合同管理

首页顶部「工作入口」业务岗直接进入 **销售合同** / **采购合同** 列表（无 `kind` 时仍回退合同总览 hub）。`SALES` / `RISK` 可在列表新建；`MANAGER` 只读、看不到新建按钮。

| 入口 | 列表口径 | 填写路径 |
| --- | --- | --- |
| **销售合同管理** | 已到达销售合同/订单确认（N3）或已有销售合同的出口案件。仍在报价（N2）的案件不出现；`DEMO-BLOCK` 已到 N3（买方筛查硬拦截）会出现在未出运，但不能推进。列表按 **未出运 / 已出运 / 已完成** 分组（H5 三段入口 + 列表筛选）。**已出运**：CIF 装运日期、FOB 到达口岸时间、电汇结算下装运日期，或 N6/提单/无提单路径。**已完成**：已出运且已提货且 **N9 已回款**（水单/到账且未收汇为 0；与占用释放同一口径）。打开后进入 N3；**本案已离开 N3** 时提供短 CTA「进入下一步」。下一步为装运、单证或收汇时，与首页出运管理 / 单证管理 / 收汇管理同一路径：装运进入可新建的批次选择再打开该批 N6；单证与收汇只选已有批次再打开该批 N7 / N9，没有批次则提示先去出运。收汇只有一个批次时直接打开该批 N9（带 `batchId`） | 打开即进入 N3。买方、收货人可改并在本页筛查。货物名称/规格可改，不锁定采购合同。**运输术语** FOB / CIF 与**结算方式** 前 T/T / 后 T/T 独立；前 T/T 只维护约定比例与约定金额，凭证在收汇管理的合同前收汇上传。后 T/T 付款天数的标签为到达目的港后付款天数。金额可选 CNY / USD。CIF 装运在 N6。**新建销售合同**：短表（标题 + 金额）调用 `POST /api/cases`（演示默认品名/目的地/25,000 USD），**打开报价（N2）** |
| **采购合同管理** | 已到达国内采购/备货（N5）或已登记采购合同/PO 的案件。打开后进入 N5，列表与表单默认只暴露采购内容。N6–N9 仍挂在同一出口案件上，不迁入采购 PO。已离开 N5 或保存/过闸后，按钮改为「去办装运（出口案）」等（按当前出口节点：装运/单证/收汇）。去办装运时先进入出运批次选择，再打开该批次 N6；不把装运后续写成采购合同的「下一节点」。销售合同管理仍用「进入下一节点」；当前步骤为装运时先进入出运批次选择，而不是无批次的空白 N6 | 打开即进入既有 **N5 采购合同** 表单；**关联销售合同**为可任选的已签销售合同列表（`GET /api/cases/:id/nodes/N5/sales-options`），不锁定本案。保存/推进须已选（`salesCaseId`，闸门 `N5_SALES_LINK` / `N5_SALES_NOT_SIGNED`）。货款 **一次性付清**（约定付款时间；不填已付货款、付款日期）或 **分期支付**（每期约定付款时间、付款比例、金额，以及本期已付、付款日）。**新建采购合同**：不调用 `POST /cases`；须点选一笔**已签订的销售合同**，打开**该案** N5（已有 PO 则编辑） |
...
N5 采购表单**不再填写「客户合同交货期」**（亦不再作为必填闸门），也**不填写收货人**。须填写 **货物名称、规格**（与销售合同分列保存）、**供应商实际交付日期**（计划交付，接口仍为 `plannedArrival`）与可选的 **实际交付日期**（接口仍为 `actualArrival`）。表单只读展示对照用的 **关联销售合同交货期**。若该交期存在，则供应商实际交付日期或实际交付日期**任一**晚于该交期即须登记延期。供应商、关联销售合同任选、付款方式（一次性付清/分期）不变。未单独保存过的采购货物，界面可暂用销售合同品名作默认；保存后两边互不覆盖。

## 客户管理

首页顶部「工作入口」进入 **客户管理**。仅收录 **已到达合同/订单确认（N3）及之后** 的买方：买方在销售合同页填写。**仍在报价（N2）的案件不会作为正式客户出现**。`DEMO-BLOCK` 已到 N3，买方会录入，但筛查硬拦截不得推进。同一买方多笔已达 N3 的订单合并到同一客户详情。
...
- 收汇节点登记 `receivedAt`（实际到账日）与 `amountFen`（已收汇金额）；有汇款附言但未填到账日则保存时记为当天。**销售列表「已完成」与占用「已回款」都只认本账本**；N3「是否收汇」只读，不能单独假装完成。
- 未收齐且到期日已过 → 逾期；已收齐且到账不晚于到期日 → 按期；未到期则未到期
- 客户总体：任一笔逾期则为逾期

演示客户 **Nordlicht GmbH**：多笔已达 N3 的订单（`DEMO-PASS` / `DEMO-NORD-LATE` / `DEMO-NORD-OPEN` / `DEMO-NORD-WIP`）按名称+国家合并为同一档案；中信保限额 150,000 USD。占用口径：`DEMO-NORD-WIP` 未履行完毕未回款 18,000；`DEMO-NORD-LATE` 已履行未回款 25,000 + `DEMO-NORD-OPEN` 已履行未回款 72,000；`DEMO-PASS` 已收齐不占用。合计占用 115,000，剩余额度 35,000。Acme（`DEMO-SOFT`）与 Cedar Trade（`DEMO-NOLIMIT`）已达 N3、尚无合同、限额未登记。Banned Trading（`DEMO-BLOCK`）已到 N3，会录入，但买方筛查硬拦截不得推进。
...
1. **每一笔采购合同/采购单**：PO 号、**关联销售合同**（客户、销售合同案件号、金额、状态）、关联出口案件、供应商实际交付日期/实际交付日期、关联销售合同交货期
2. **是否按期交货**：实际交付日期对照供应商实际交付日期（无供应商实际交付日期则对照关联销售合同交货期）→ 按期 / 逾期 / 未到期 / 无交货记录
...
报价含模糊用语、变更单未确认、采购未关联已签销售合同、采购交付延期无客户同意，分别对 N2 / N4 / N5 `advance` 同样拒绝。N7 缺六份单证，或非 FOB 缺原产地证，`advance` 返回 **409**。国内供应商高置信命中时 N5 `advance` 返回 **409** 且 `missing` 含 `N5_HIGH_CONFIDENCE_HIT`。未关联销售合同时 N5 保存或推进返回 **409**，`missing` 含 `N5_SALES_LINK`（或关联对象尚未签订销售合同时为 `N5_SALES_NOT_SIGNED`）。进入 N4 后若未再次确认中信保，或变更后占用属超高风险，N4 `advance` 也会拒绝。高风险占用进入工作台审核队列（`REVIEW`，`missing` 含 `N3_SINOSURE_EXPOSURE_HIGH` / `N4_SINOSURE_EXPOSURE_HIGH`）：`GET /api/workbench/queue` 可见占用高风险任务，`POST /api/workbench/:caseId/action` 传 `reviewId` + `CLAIM` / `APPROVE` / `REJECT`。**放行后** N3/N4 可推进；**驳回后**仍阻断；**无需改合同金额**。超高风险不进工作台。无变更单时 N3 通过后直接进入 N5，不要求 N4 中信保。

常用接口：

- `GET /api/users` 演示用户（含 `role` / `roleLabel`：SALES 业务、RISK 风控、MANAGER 主管）
- `POST /api/cases` 新建出口案（从 N2 报价起；可选 `buyerName` / `buyerCountry` 预填买方/收货人，在 N3 可改）。业务岗「新建销售合同」走此接口并打开报价；「新建采购合同」不走此接口，改为点选已签销售合同
...
- `GET /api/catalog` 节点、HS 模板、延期原因、所含项目/单价单位等
- `POST /api/cases/:id/nodes/N3/screen` 买方/收货人模拟筛查（原询盘 KYC，须在销售合同推进前通过）
...
- `POST /api/cases/:id/nodes/N5/plan` 采购合同/国内备货（须传 `salesCaseId` 关联用户选定的已签销售合同；供应商、PO、供应商实际交付日期、付款方式一次性付清或分期支付）。分期支付每期须填 `dueAt` 或 `conditionText`（约定付款时间）、`percent`（付款比例）、`amountFen`（金额）。未关联或关联对象未签销售合同则 **409 GATE_REFUSED**；不会因本案已签而自动填入
- `POST /api/cases/:id/nodes/N5/screen` 国内供应商模拟筛查
- `POST /api/cases/:id/nodes/N6/docs/:slot/upload` 装运商业发票 `invoice` / 箱单 `packing`（证据链；可传演示 PDF。槽位键仍为 `invoice`）
- `POST /api/cases/:id/nodes/N7/docs/:slot/upload` 单证：`sales-contract`、`commercial-invoice`、`packing`、`purchase-contract`、`invoice`、`customs`、`origin-cert`（原产地证；FOB 可不传）
- `GET /api/cases/:id/nodes/:code/gate` 预览闸门
- `POST /api/cases/:id/nodes/:code/advance` 过闸
- `GET /api/workbench/queue` 审核队列（筛查命中 + 占用高风险 + **退税·融资性** `TAX_FINANCE`）
- `POST /api/workbench/:caseId/action` 工作台处置（筛查命中用 `hitId`；占用高风险用 `reviewId` + `CLAIM`/`APPROVE`/`REJECT`；退税·融资性同一套动作，黄灯可领取/通过/驳回，红线 `HARD_BLOCKED` 不可放行）
- `POST /api/cases/:id/tax-rebate` 保存退税就绪清单（进项发票号可手填 + 四流勾选）
- `POST /api/cases/:id/tax-rebate/declare` 模拟申报退税（FT4 未齐 **409**）
...
- **当事方关系**：买方、收货人在 N3 填写（可与买方不同）；付款人不是必填；**国内供应商**（N5 采购对象，须单独筛查）。采购合同不填写收货人
...
- **无提单路径**：FOB / EXW / FAS / FCA 等买方安排运输时，不要求提单号与正本/电放；须选择「无提单」。不再要求装船通知 / 订舱编号。CIF/CFR 等卖方出单仍须正本或电放。合同术语由 N3 带入 N6，可在 N6 手工改术语后过闸。N7 不跟提单或订舱号：六份必填，非 FOB 另须原产地证，过闸后进入收汇。
- **工作台动作**：筛查命中为误报排除、确认真实、补充信息、持续监控；占用高风险为领取、放行、驳回；退税·融资性黄灯为领取、通过、驳回（写入审计）。三页不混列：额度审核 | 制裁命中 | 退税·融资性审核。仅风控岗可写。
- **交货方式**：销售合同必选自有仓 / **港口直出**（不再选保税）。港口直出须填货物仓储地点、批次号，否则 409。薄利+直出黄灯进工作台；像空转或购销/报关货描严重不符为红线硬拦截。退税申报前须 FT4 就绪清单。
...
- **币种**：销售合同 / 出口案件金额与中信保占用 **固定 USD**（界面锁定，保存时校验）；采购合同 / PO 金额 **固定 CNY**。占用与分档只按美元加总，**不自动换算**。限额币种须为美元并与合同一致。客户评估毛利因购销币种不同不换算，展示「待合同毛利率表」。

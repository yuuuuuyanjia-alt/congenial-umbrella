...
| 案件号 | 路径 | 说明 |
| --- | --- | --- |
| `DEMO-PASS` | 绿灯通过 | Nordlicht GmbH，清单未命中；国内供应商苏州精工机械筛查通过；CIF **正本提单**过 N6；报价 v1 废止 / v2 生效；中信保限额 150,000 USD；变更单 CO-001 数量 8→10 已确认生效并再次核对占用；**采购合同 `PO-BH-2026-011` 关联销售合同 DEMO-PASS**；采购到货未延期；HS 要素与原产地证齐全并模拟放行；**销售合同已填 CIF 装运港 Shanghai、装运日期、预计到港 Hamburg 与已提货**；**N9 水单已收汇全额**，销售列表 **已完成**，占用已释放 |
...
N5 采购表单**不再填写「客户合同交货期」**（亦不再作为必填闸门）。须填写 **供应商实际交付日期**（原计划到货/备妥，接口仍为 `plannedArrival`）与可选的 **实际交付日期**（原实际到货，接口仍为 `actualArrival`）。若关联销售合同有交货期，则对照该交期判断是否须登记延期。供应商、关联销售合同任选、付款方式（一次性付清/分期）不变。

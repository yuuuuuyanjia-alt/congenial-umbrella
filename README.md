...
| N6 | 装运/提单指示 | **硬闸门**：上传发票 + 上传箱单（证据链）+ 内部审批。不再要求客户书面指示或订舱编号。CIF 填装运港口、装运日期、预计到港。装运规则跟随所选**运输术语**：CIF/CFR 等须 **正本或电放其一**；FOB/EXW/FAS/FCA 可走 **无提单**（须填写原因）。T/T 是结算方式不是 Incoterm；无有效运输术语时**回退按 FOB**。过闸后本页「进入下一步」 |
...
| `DEMO-FOB` | FOB 无提单 | Pacific Tools，FOB 买方订舱；N3 已填国内段到达口岸/港口时间（2026-12-08 10:00，完成国内交付）；N6 走「无提单」路径（发票、箱单与内部审批，不要求书面指示或订舱编号），已过闸停在 N7。N7 须再上传销售合同、商业发票、箱单、采购合同、发票、报关单六份才能推进。采购 `PO-PT-FOB-004` **关联销售合同 DEMO-FOB**。销售列表归入 **已出运** |
...
- `POST /api/cases/:id/nodes/N6/shipment/upload` 装运发票或箱单（multipart `file` + `kind`=`INVOICE`|`PACKING`）
- `POST /api/cases/:id/nodes/N7/docs/upload` 单证一致性六份之一（multipart `file` + `kind`=`SALES_CONTRACT`|`COMMERCIAL_INVOICE`|`PACKING`|`PURCHASE_CONTRACT`|`INVOICE`|`CUSTOMS`）
...
- **无提单路径**：FOB / EXW / FAS / FCA 等买方安排运输时，不要求提单号与正本/电放；须选择「无提单」并填写原因。不再要求装船通知 / 订舱编号。CIF/CFR 等卖方出单仍须正本或电放。合同术语由 N3 带入 N6，可在 N6 手工改术语后过闸。N7 不跟提单或订舱号，只认六份上传。

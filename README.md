...
| --- | --- | --- | --- | --- |
| 业务 | `SALES` | 业务 | 客户 / 销售 / 采购录入，N1–N9 过闸。工作台领取/放行/驳回隐藏且接口拒绝 | 合同、供应商、客户（无工作台入口） |
| 风控 | `RISK` | 风控 | 工作台领取/放行/驳回与筛查处置。可查看合同；未扩大金额改写 | 工作台优先，其后合同/客户/供应商 |
| 主管 | `MANAGER` | 主管 | 本轮只读（评估、占用、列表）。不可审批、推进、改合同 | 客户、合同优先（无工作台入口） |

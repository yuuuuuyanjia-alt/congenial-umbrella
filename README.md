...
| N7 | 单证一致性 | **硬闸门**：销售合同、商业发票、箱单、采购合同、发票、报关单六份必填；**原产地证**在运输术语不是 FOB 时必填（FOB 可不传）。**商业发票与发票分别必填**，缺则 **409**。不再核提单、装船通知或字段勾选，也不再核 HS/申报要素。过闸后本页「进入下一步」进入 **N9 收汇** |
...
首页工作入口包含 **出运管理**、**单证管理** 与 **费用管理**（业务、风控、主管都能看到；主管只读）。收汇（N9）仍留在出运批次主链上，不放到首页。

- **出运管理**：先选销售合同，再选择或新建出运批次，进入该批次的装运/提单指示（N6）。选定后用本页「进入下一步」到 N7、N9。
- **单证管理**：先选销售合同，再选择**已有**批次，进入该批次的单证一致性（N7）。没有批次时提示先去出运，不从单证新建空批次，也不打开没有 `batchId` 的空白单证页。
...
- 销售合同列表「下一步」若是装运或单证，与上面同一路径（合同已选定，直接进入对应的批次选择）。案件详情里点 N6/N7/N9 同样先选批次，不再跳进空白节点页。

验证：销售合同详情没有「出运批次」区块。首页能看到出运管理、单证管理。出运：选销售合同 → 选择或新建批次 → 该批 N6。单证：选销售合同 → 只选已有批次 → 该批 N7；没有批次时提示去出运。对已到装运的案件点列表「进入下一步」，应先看到批次选择/新建，选定后再进入该批 N6。下一步若是单证，则只列出已有批次。再新建一批并分别进入两批的 N6。推进其中一批不应要求另一批先完成。
...
| --- | --- | --- | --- | --- |
| 业务 | `SALES` | 业务 | 客户 / 销售 / 采购录入，N2–N9 过闸。工作台领取/放行/驳回隐藏且接口拒绝 | **销售合同、采购合同**，其后 **出运管理、单证管理**，再供应商、客户（无工作台入口；收汇不在首页） |
| 风控 | `RISK` | 风控 | 工作台领取/放行/驳回与筛查处置。可查看合同；未扩大金额改写 | 工作台优先，其后销售合同/采购合同、出运管理/单证管理、客户/供应商 |
| 主管 | `MANAGER` | 主管 | 本轮只读（评估、占用、列表）。不可审批、推进、改合同或新建 | 客户、销售合同、采购合同优先，其后出运管理/单证管理（无工作台入口） |
...
| 入口 | 列表口径 | 填写路径 |
| --- | --- | --- |
| **销售合同管理** | 已到达销售合同/订单确认（N3）或已有销售合同的出口案件。仍在报价（N2）的案件不出现；`DEMO-BLOCK` 已到 N3（买方筛查硬拦截）会出现在未出运，但不能推进。列表按 **未出运 / 已出运 / 已完成** 分组（H5 三段入口 + 列表筛选）。**已出运**：CIF 装运日期、FOB 到达口岸时间、电汇结算下装运日期，或 N6/提单/无提单路径。**已完成**：已出运且已提货且 **N9 已回款**（水单/到账且未收汇为 0；与占用释放同一口径）。打开后进入 N3；**本案已离开 N3** 时提供短 CTA「进入下一步」。下一步为装运或单证时，与首页出运管理 / 单证管理同一路径：装运进入可新建的批次选择再打开该批 N6；单证只选已有批次再打开该批 N7，没有批次则提示先去出运 | 打开即进入 N3。买方、收货人可改并在本页筛查。货物名称/规格可改，不锁定采购合同。**运输术语** FOB / CIF 与**结算方式** 前 T/T / 后 T/T 独立；后 T/T 付款天数的标签为到达目的港后付款天数。金额可选 CNY / USD。CIF 装运在 N6。**新建销售合同**：短表（标题 + 金额）调用 `POST /api/cases`（演示默认品名/目的地/25,000 USD），**打开报价（N2）** |

...
| N6 | 装运/提单指示 | **硬闸门**：客户书面指示 + 内部审批。CIF 填装运港口、装运日期、预计到港。装运规则跟随所选**运输术语**：CIF/CFR 等须 **正本或电放其一**；FOB/EXW/FAS/FCA 可走 **无提单**（须记录依据）。T/T 是结算方式不是 Incoterm；无有效运输术语时**回退按 FOB** |
| N7 | 单证一致性 | **硬闸门**：终稿合同 + 合同/发票/装箱单字段一致 + 不符点修改记录。CIF/卖方提单路径另须提单一致；**FOB 等 N6 无提单路径改核装船通知/订舱号，不硬要提单**。运输术语按 Incoterms 比对，T/T 结算方式不参与；装运规则与 N6 相同 |
...
覆盖：N3 买方筛查高置信硬拦截 / 低置信软提示、**无付款人亦可过闸**、未筛查不得推进；N2 模糊报价拒绝、所含项目/单价单位、成本底线偏离软提示；N3 缺条款拒绝、**中信保限额未登记不得签订合同 / 缺保单/占用超高风险拒绝、高风险工作台领取/放行/驳回、中风险软提示**、**CIF/CIP 装运节点**、**交货方式（不强制自有仓）与港口直出四问（FT1/FT2）**；N4 变更确认链、**变更后占用复核（高风险同样工作台审核）**、无变更跳过且不要求 N4 中信保、收货人被清空时重跑买方筛查；N5 **须关联已签销售合同**、供应商高置信硬拦截、采购交付延期无同意中风险、**购销匹配与薄利+直出黄灯（FT1）**、**货物名称/规格与销售合同分列**；**N6 正本/电放二选一、FOB 无提单路径、缺证据拒绝**；N7 / N9 缺证据拒绝推进；N8 HS/申报要素缺口禁止申报、**直出货描严重不符红线（FT3）**；**采购货款一次性付清或分期支付（每期约定付款时间、付款比例、金额）与按期次逾期**；**先销售后采购：未签销售合同不得作为采购关联对象，已签合同可任选、不自动锁定本案**；**销售/采购合同列表按 N3 / N5 过滤，不混用采购 PO 作为销售合同条目**；**已回款单一事实源：列表已完成与占用释放均认 N9 水单/到账，勾选 N3 是否收汇不能假装完成**；**退税就绪清单 FT4（报关放行、N9 收汇、进项发票号、四流勾选）**；**新建销售合同从 N2 报价起，再到 N3；新建采购合同须点选已签销售合同后打开该案 N5**。
...
| `DEMO-FOB` | FOB 无提单 | Pacific Tools，FOB 买方订舱；N3 已填国内段到达口岸/港口时间（2026-12-08 10:00，完成国内交付）；N6 走「无提单」路径（书面指示 + 内部审批 + 装船通知依据），已过闸停在 N7（合同/发票/装箱单已备，**无提单**；有装船通知/订舱号即可过 N7）；采购 `PO-PT-FOB-004` **关联销售合同 DEMO-FOB**。销售列表归入 **已出运** |
...
- **无提单路径**：FOB / EXW / FAS / FCA 等买方安排运输时，不要求提单号与正本/电放；须选择「无提单」并记录装船通知 / 订舱 / 买方运输安排。N7 跟随 N6：无提单路径核验装船通知/订舱号，不硬要提单；CIF/卖方提单路径仍须提单一致。CIF/CFR 等卖方出单仍须正本或电放。合同术语由 N3 带入 N6，可在 N6 手工改术语后过闸。

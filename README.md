...
| N6 | 装运/提单指示 | **硬闸门**：上传发票 + 上传箱单（证据链）+ 内部审批。不再要求客户书面指示或订舱编号。CIF 填装运港口、装运日期、预计到港。装运规则跟随所选**运输术语**：CIF/CFR 等须 **正本或电放其一**；FOB/EXW/FAS/FCA 可走 **无提单**。T/T 是结算方式不是 Incoterm；无有效运输术语时**回退按 FOB**。过闸后本页「进入下一步」 |
...
| `DEMO-GATE` | 闸门演示 | 已过 KYC / 报价 / **销售合同**（FOB，含中信保）/ **无变更故跳过 N4** / 国内采购备货（`PO-HV-2026-088` **关联 DEMO-GATE**），停在 N6，发票与箱单尚未上传 |
...
- `POST /api/cases/:id/nodes/N6/docs/:slot/upload` 装运发票 `invoice` / 箱单 `packing`（证据链；可传演示 PDF）

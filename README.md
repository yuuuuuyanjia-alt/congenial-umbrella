...
| N3 | 销售合同/订单确认 | **销售/出口合同**与采购合同分开签订。贸易条件 **FOB / CIF / T/T 三选一**：**CIF** 填装运港口、装运日期、预计到港；**FOB** 填国内段**到达口岸/港口时间**；**T/T** 再选 **前 T/T** 或 **后 T/T** 并填写对应收汇节点。**客户是否提货**与收汇金额在所选路径下可填。销售合同列表按 **未出运 / 已出运 / 已完成** 分组 |
...
覆盖：N1 高置信硬拦截 / 低置信软提示；N2 模糊报价拒绝与成本底线偏离；N3 缺条款拒绝、**中信保限额未登记不得签订合同 / 缺保单/占用超高风险拒绝、高风险审核、中风险软提示**、**CIF/CIP 装运节点与收汇金额轧差**；N4 变更确认链、**变更后占用复核**、无变更跳过且不要求 N4 中信保；N5 **须关联已签销售合同**、供应商高置信硬拦截、采购到货延期无同意中风险；**N6 正本/电放二选一、FOB 无提单路径、缺证据拒绝**；N7 / N9 缺证据拒绝推进；N8 HS/申报要素缺口禁止申报；**采购货款一次性付清或分期支付（每期约定付款时间、付款比例、金额）与按期次逾期**；**先销售后采购：未签销售合同不得作为采购关联对象，已签合同可任选、不自动锁定本案**；**销售/采购合同列表按 N3 / N5 过滤，不混用采购 PO 作为销售合同条目**。
...
| 案件号 | 路径 | 说明 |
| --- | --- | --- |
| `DEMO-PASS` | 绿灯通过 | Nordlicht GmbH，清单未命中；国内供应商苏州精工机械筛查通过；CIF **正本提单**过 N6；报价 v1 废止 / v2 生效；中信保限额 150,000 USD；变更单 CO-001 数量 8→10 已确认生效并再次核对占用；**采购合同 `PO-BH-2026-011` 关联销售合同 DEMO-PASS**；采购到货未延期；HS 要素与原产地证齐全并模拟放行；**销售合同已填 CIF 装运港 Shanghai、装运日期、预计到港 Hamburg 与已提货，以及约定付款日与已收汇全额** |
...
| `DEMO-NORD-WIP` | 未履行完毕 | Nordlicht **后 T/T** 已签销售合同停在国内采购，尚未装运，未回款 18,000 USD；**待登记采购合同并关联本销售案件** |
...
| 入口 | 列表口径 | 填写路径 |
| --- | --- | --- |
| **销售合同管理** | 已到达销售合同/订单确认（N3）或已有销售合同的出口案件。询盘未通过（如 `DEMO-BLOCK`）不出现。列表按 **未出运 / 已出运 / 已完成** 分组（H5 三段入口 + 列表筛选）。**已出运**：CIF 装运日期、FOB 到达口岸时间、T/T 装运日期，或 N6/提单/无提单路径。**已完成**：已出运且已提货且已收汇且未收汇为 0。演示：`DEMO-NORD-WIP`（后 T/T）/`DEMO-HELIOS-WIP`（前 T/T）未出运，`DEMO-NORD-OPEN` / `DEMO-FOB` / `DEMO-HELIOS-DONE` 已出运，`DEMO-PASS` / `DEMO-NORD-LATE` 已完成。打开后进入 N3；**本案已离开 N3** 时列表与表单提供「进入下一节点」，直达当前九节点（N3 无变更单则下一步为 N5）。过闸成功后自动打开下一节点 | 打开即进入 N3。贸易条件 **FOB / CIF / T/T** 三选一，所选路径字段可填 |
...
- `GET /api/cases` 案件列表；`?kind=sales` 销售合同列表（已达 N3 或已有销售合同；条目含 `shipmentBucket`：`unshipped` 未出运 / `shipped` 已出运 / `completed` 已完成）；`?kind=procurement` 采购合同列表（已达 N5 或已有采购 PO，条目含 `procurementTitle`：供应商采购产品出口客户）

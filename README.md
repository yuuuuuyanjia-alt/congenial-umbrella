...
- **费用管理**：先选销售合同，再登记该合同的费用。固定项为海运费、陆运费、港杂、保险，另可增删自定义费用（名称 + 金额）。全部选填，可以整单留空保存。数据在 `ContractFee`（`caseId` 唯一），币种跟随销售合同；合同还没有币种时标明 USD 或 CNY。不按出运批次，也不进入 N 节点闸门。报价「所含项目」仍只在 N2 多选，本页只把最新报价的所含项目当作提示，不回写报价。接口：`GET/POST /api/cases/:id/fees`。

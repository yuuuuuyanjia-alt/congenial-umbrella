...
| 节点 | 名称 | 要点 |
| --- | --- | --- |
| N1 | 询盘/客户KYC | 买方 / 收货人；模拟筛查 OFAC、UN、EU、UK、中国不可靠实体清单；KYC 报告 + 风险评分；高置信命中硬拦截。付款人不再作为询盘必填项 |
| N2 | 报价环节 | 所含项目（海运费 / 陆运费 / 港杂多选）、有效期、美元单价（吨或千克）；「价格待定/费用另议」禁止推进；相对成本底线/历史价异常偏离软提示；报价版本 + 字段快照入审计 |
| N3 | 销售合同/订单确认 | **销售/出口合同**与采购合同分开签订。金额 **固定美元**。**运输术语**（FOB / CIF 及 CIP 等）与**结算方式**（前 T/T / 后 T/T）独立，可组合例如 **FOB + 前 T/T**。**CIF** 填装运港口、装运日期、预计到港；**FOB** 填国内段**到达口岸/港口时间**；电汇填对应收汇节点。T/T **不得**写入 `incoterms`。**客户是否提货**可填；**是否收汇 / 收汇金额 / 未收汇只读**，以 N9 水单/到账为唯一事实源。销售合同列表按 **未出运 / 已出运 / 已完成** 分组 |
| N4 | 变更管理 | 交货期 / 数量 / 收货人 / 付款条件变更须出变更单（含 diff）；客户确认 + 内部确认；敏感变更须审批；旧版 `SUPERSEDED`；当事方/付款/收货人变更会重跑关联节点闸门；**进入变更时须再次确认中信保并按变更后金额重算占用** |
| N5 | 采购合同/国内备货 | **先销售后采购**：须关联一笔已达 N3 且已签的销售合同案件（客户、合同号、金额、状态），否则不得保存或推进；采购合同/PO 金额 **固定人民币**；登记国内供应商、采购合同/PO、**供应商实际交付日期**与可选**实际交付日期**；付款方式选择 **一次性付清** 或 **分期支付**（每期须填约定付款时间、付款比例、金额）；供应商过 OFAC/UN/EU/UK 与中国不可靠实体模拟筛查（高置信硬拦截、中置信审核队列、低置信软提示）；供应商实际交付日期或实际交付日期**任一**晚于**关联销售合同交货期**，须登记**结构化延期触发条件**；表单展示对照用的销售合同交货期；客户同意延期须有可追溯证据编号；无同意则中风险不得推进 |
| N6 | 装运/提单指示 | **硬闸门**：客户书面指示 + 内部审批。装运规则跟随所选**运输术语**：CIF/CFR 等须 **正本或电放其一**；FOB/EXW/FAS/FCA 可走 **无提单**（须记录依据）。T/T 是结算方式不是 Incoterm；无有效运输术语时**回退按 FOB**（买方安排运输），避免把 `T/T` 切成 `T` 后误走卖方提单 |
...
| --- | --- | --- | --- | --- |
| 业务 | `SALES` | 业务 | 客户 / 销售 / 采购录入，N1–N9 过闸。工作台领取/放行/驳回隐藏且接口拒绝 | **销售合同、采购合同**、供应商、客户（无工作台入口） |
...
覆盖：N1 高置信硬拦截 / 低置信软提示、**无付款人亦可过闸**；N2 模糊报价拒绝、所含项目/单价单位、成本底线偏离软提示；N3 缺条款拒绝、**中信保限额未登记不得签订合同 / 缺保单/占用超高风险拒绝、高风险工作台领取/放行/驳回、中风险软提示**、**CIF/CIP 装运节点**、**交货方式（不强制自有仓）与港口直出四问（FT1/FT2）**；N4 变更确认链、**变更后占用复核（高风险同样工作台审核）**、无变更跳过且不要求 N4 中信保；N5 **须关联已签销售合同**、供应商高置信硬拦截、采购交付延期无同意中风险、**购销匹配与薄利+直出黄灯（FT1）**；**N6 正本/电放二选一、FOB 无提单路径、缺证据拒绝**；N7 / N9 缺证据拒绝推进；N8 HS/申报要素缺口禁止申报、**直出货描严重不符红线（FT3）**；**采购货款一次性付清或分期支付（每期约定付款时间、付款比例、金额）与按期次逾期**；**先销售后采购：未签销售合同不得作为采购关联对象，已签合同可任选、不自动锁定本案**；**销售/采购合同列表按 N3 / N5 过滤，不混用采购 PO 作为销售合同条目**；**已回款单一事实源：列表已完成与占用释放均认 N9 水单/到账，勾选 N3 是否收汇不能假装完成**；**退税就绪清单 FT4（报关放行、N9 收汇、进项发票号、四流勾选）**；**新建销售合同从 N1 询盘 KYC 起，沿主链走到 N3；新建采购合同须点选已签销售合同后打开该案 N5**。
...
| 入口 | 列表口径 | 填写路径 |
| --- | --- | --- |
| **销售合同管理** | 已到达销售合同/订单确认（N3）或已有销售合同的出口案件。询盘未通过（如 `DEMO-BLOCK`）不出现。列表按 **未出运 / 已出运 / 已完成** 分组（H5 三段入口 + 列表筛选）。**已出运**：CIF 装运日期、FOB 到达口岸时间、电汇结算下装运日期，或 N6/提单/无提单路径。**已完成**：已出运且已提货且 **N9 已回款**（水单/到账且未收汇为 0；与占用释放同一口径）。勾选 N3「是否收汇」不能假装完成。演示：`DEMO-NORD-WIP`（CIF + 后 T/T）/`DEMO-HELIOS-WIP`（FOB + 前 T/T）未出运，`DEMO-NORD-OPEN` / `DEMO-FOB` / `DEMO-HELIOS-DONE` / `DEMO-NORD-LATE` 已出运，`DEMO-PASS` 已完成。打开后进入 N3；**本案已离开 N3** 时列表与表单提供「进入下一节点」，直达当前九节点（N3 无变更单则下一步为 N5）。过闸成功后自动打开下一节点 | 打开即进入 N3。**运输术语** FOB / CIF 与**结算方式** 前 T/T / 后 T/T 独立勾选，字段均可填；收汇金额只读。**新建销售合同**：短表（标题 + 金额）调用 `POST /api/cases`（演示默认品名/目的地/25,000 USD，买方与收货人同名预填），**打开询盘/客户 KYC（N1）**。用户沿 N1 筛查 → N2 报价 → N3 销售合同办理，不跳到空白 N3 |
...
N5 采购表单**不再填写「客户合同交货期」**（亦不再作为必填闸门）。须填写 **供应商实际交付日期**（计划交付，接口仍为 `plannedArrival`）与可选的 **实际交付日期**（接口仍为 `actualArrival`）。表单只读展示对照用的 **关联销售合同交货期**。若该交期存在，则供应商实际交付日期或实际交付日期**任一**晚于该交期即须登记延期。供应商、关联销售合同任选、付款方式（一次性付清/分期）不变。
...
- `POST /api/cases` 新建出口案（从 N1 起；可选 `buyerName` / `buyerCountry` 预填买方/收货人）。业务岗「新建销售合同」走此接口并打开 KYC；「新建采购合同」不走此接口，改为点选已签销售合同
...
- **当事方关系**：买方、收货人（可与买方不同）；付款人不再作为询盘必填；**国内供应商**（N5 采购对象，须单独筛查）
...
- **交货方式**：销售合同必选自有仓 / 保税 / **港口直出**，**不强制自有仓**。N5 只读同步关联销售合同。港口直出须四问+证据（货在、报关主体、收汇绑定、是否像空转）；缺则 409。薄利+直出黄灯进工作台；像空转或购销/报关货描严重不符为红线硬拦截。退税申报前须 FT4 就绪清单。

...
| N3 | 销售合同/订单确认 | **销售/出口合同**与采购合同分开签订。**所有权保留**、**争议解决**条款必填；校验 Incoterms 与付款条件；记录交货期与数量；**中信保限额未登记，不得签订合同**；须上传中信保保单并登记投保限额；按买方占用测算，超额分档提示或拦截；**CIF/CIP** 可填装运港口、装运日期、预计到达日期与到达港口；**FOB** 等填国内段**到达口岸/港口时间**（到达即完成国内交付）；**客户是否提货**与约定客户付款日期、是否收汇、收汇/未收汇金额适用于全部术语（未收汇=合同总额−收汇金额）。销售合同列表按 **未出运 / 已出运 / 已完成** 分组 |
...
| 入口 | 列表口径 | 填写路径 |
| --- | --- | --- |
| **销售合同管理** | 已到达销售合同/订单确认（N3）或已有销售合同的出口案件。询盘未通过（如 `DEMO-BLOCK`）不出现。列表按 **未出运 / 已出运 / 已完成** 分组（H5 三段入口 + 列表三栏分区）。**已出运**：CIF 装运日期已填，或 FOB 国内段到达口岸/港口时间已填，或 N6 已通过/已过装运节点，或已有提单号 / FOB 无提单路径。**已完成**：已出运且 `客户是否提货` 为已提货且已收汇且未收汇金额为 0。已出运列不含已完成。打开卡片仍进 N3 表单。演示：`DEMO-NORD-WIP` 未出运，`DEMO-NORD-OPEN` / `DEMO-FOB` / `DEMO-HELIOS-DONE` 已出运，`DEMO-PASS` / `DEMO-NORD-LATE` 已完成 | 打开即进入既有 **N3 销售合同/订单确认** 表单，可保存并提交。CIF/CIP 显示装运港口、装运日期、预计到港货物状态；FOB 显示国内段到达口岸/港口时间；**客户是否提货**与收汇适用于全部术语 |

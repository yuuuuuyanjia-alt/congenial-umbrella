...
| N6 | 装运/提单指示 | **硬闸门**：上传商业发票 + 上传箱单（证据链）+ 内部审批。不再要求客户书面指示或订舱编号。CIF 填装运港口、装运日期、预计到港。装运规则跟随所选**运输术语**：CIF/CFR 等须 **正本或电放其一**；FOB/EXW/FAS/FCA 可走 **无提单**。T/T 是结算方式不是 Incoterm；无有效运输术语时**回退按 FOB**。过闸后若下一步是单证，本页只「进入下一步」到 N7，不另开「去收汇」 |
...
- **出运管理**：先选销售合同，再选择或新建出运批次，进入该批次的装运/提单指示（N6）。装运硬闸门通过后，若下一步是单证，只用「进入下一步」到 N7，不旁路「去收汇」。下一步已经是收汇（N8/N9）时，同一按钮才写成「去收汇」，并带上当前批次。
- **单证管理**：先选销售合同，再选择**已有**批次，进入该批次的单证一致性（N7）。没有批次时提示先去出运，不从单证新建空批次，也不打开没有 `batchId` 的空白单证页。单证硬闸门通过后，「去收汇」打开**同一批次**的 N9。
- **收汇管理**：先选销售合同。选定后即可办理**合同前收汇**：只上传/管理前 T/T 凭证（按合同保存，不绑定批次；还没有批次也可以上传）。约定比例、约定金额只读，来自 N3，保存凭证不会回写这些条款。再选择**已有**批次，进入该批次的收汇对账（N9）。没有批次时提示先去出运，不新建批次，也不打开没有 `batchId` 的空白收汇页。直接打开 N9 且不带 `batchId` 时，落到与单证相同的批次选择页。N9 页头只读展示合同号、客户、批次、本批金额，以及 N3 的前/后 T/T 摘要（约定比例、约定金额、到达目的港后付款天数）。一批次一笔收汇；合并付款仍由业务员按批次拆开。闸门规则本轮不改。
...
验证：销售合同详情没有「出运批次」区块。首页能看到出运管理、单证管理、收汇管理。出运：选销售合同 → 选择或新建批次 → 该批 N6。单证：选销售合同 → 只选已有批次 → 该批 N7；没有批次时提示去出运。收汇：选销售合同 → 合同前收汇只上传凭证（比例与约定金额只读）；只选已有批次 → 该批 N9；没有批次时提示去出运，仍可上传合同前收汇凭证。N3 不再有前 T/T 凭证上传。N6 过闸且下一步是 N7 时只进入单证，没有「去收汇」。N7 过闸后，或下一步已经是 N9 时，「去收汇」进入同一批次 N9。对已到装运的案件点列表「进入下一步」，应先看到批次选择/新建，选定后再进入该批 N6。下一步若是单证，则只列出已有批次。下一步若是收汇，唯一批次直接进入该批 N9。再新建一批并分别进入两批的 N6。推进其中一批不应要求另一批先完成。

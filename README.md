...
| N3 | 销售合同/订单确认 | **销售/出口合同**与采购合同分开签订。贸易条件 **FOB / CIF / T/T 三选一**：**CIF** 填装运港口、装运日期、预计到港；**FOB** 填国内段**到达口岸/港口时间**；**T/T** 再选 **前 T/T** 或 **后 T/T** 并填写对应收汇节点。**客户是否提货**可填；**是否收汇 / 收汇金额 / 未收汇只读**，以 N9 水单/到账为唯一事实源。销售合同列表按 **未出运 / 已出运 / 已完成** 分组 |
...
| 入口 | 列表口径 | 填写路径 |
| --- | --- | --- |
| **销售合同管理** | 已到达销售合同/订单确认（N3）或已有销售合同的出口案件。询盘未通过（如 `DEMO-BLOCK`）不出现。列表按 **未出运 / 已出运 / 已完成** 分组（H5 三段入口 + 列表筛选）。**已出运**：CIF 装运日期、FOB 到达口岸时间、T/T 装运日期，或 N6/提单/无提单路径。**已完成**：已出运且已提货且 **N9 已回款**（水单/到账且未收汇为 0；与占用释放同一口径）。勾选 N3「是否收汇」不能假装完成。演示：`DEMO-NORD-WIP`（后 T/T）/`DEMO-HELIOS-WIP`（前 T/T）未出运，`DEMO-NORD-OPEN` / `DEMO-FOB` / `DEMO-HELIOS-DONE` / `DEMO-NORD-LATE` 已出运，`DEMO-PASS` 已完成 | 打开即进入 N3。贸易条件 **FOB / CIF / T/T** 三选一，所选路径字段可填；收汇金额只读 |

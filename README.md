...
N3 之后，一份销售合同可以有多笔 **出运批次**。每批独立批次号、数量/金额、提单路径（N6）、单证（N7）和收汇（N9），多批可以同时在途。买方、运输术语、币种、中信保限额仍在销售合同上。采购合同（N5）本轮不按批次拆 PO。中信保占用 = 各批次未收汇 + 尚未出运的合同余额（仍只把美元计入美元占用池）。已有案件迁移为默认批次 `1`，演示路径保持单批。

验证第二批：打开已到 N3 的销售案件 →「出运批次」→ 填写批次号、数量、金额 →「新建批次」→ 分别进入两批的 N6。推进其中一批不应要求另一批先完成。
...
| 入口 | 列表口径 | 填写路径 |
| --- | --- | --- |
| **销售合同管理** | 已到达销售合同/订单确认（N3）或已有销售合同的出口案件。仍在报价（N2）的案件不出现；`DEMO-BLOCK` 已到 N3（买方筛查硬拦截）会出现在未出运，但不能推进。列表按 **未出运 / 已出运 / 已完成** 分组（H5 三段入口 + 列表筛选）。**已出运**：CIF 装运日期、FOB 到达口岸时间、电汇结算下装运日期，或 N6/提单/无提单路径。**已完成**：已出运且已提货且 **N9 已回款**（水单/到账且未收汇为 0；与占用释放同一口径）。打开后进入 N3；**本案已离开 N3** 时提供短 CTA「进入下一步」 | 打开即进入 N3。买方、收货人可改并在本页筛查。货物名称/规格可改，不锁定采购合同。**运输术语** FOB / CIF 与**结算方式** 前 T/T / 后 T/T 独立；金额可选 CNY / USD。CIF 装运在 N6。**新建销售合同**：短表（标题 + 金额）调用 `POST /api/cases`（演示默认品名/目的地/25,000 USD），**打开报价（N2）** |
| **采购合同管理** | 已到达国内采购/备货（N5）或已登记采购合同/PO 的案件。打开后进入 N5，列表与表单默认只暴露采购内容。N6–N9 仍挂在同一出口案件上，不迁入采购 PO。已离开 N5 或保存/过闸后，按钮改为「去办装运（出口案）」等（按当前出口节点：装运/单证/收汇），跳到出口案对应页，不把装运后续写成采购合同的「下一节点」。销售合同管理仍用「进入下一节点」直达本案当前步骤 | 打开即进入既有 **N5 采购合同** 表单；**关联销售合同**为可任选的已签销售合同列表（`GET /api/cases/:id/nodes/N5/sales-options`），不锁定本案。保存/推进须已选（`salesCaseId`，闸门 `N5_SALES_LINK` / `N5_SALES_NOT_SIGNED`）。货款 **一次性付清** 或 **分期支付**（每期约定付款时间、付款比例、金额）。**新建采购合同**：不调用 `POST /cases`；须点选一笔**已签订的销售合同**，打开**该案** N5（已有 PO 则编辑） |

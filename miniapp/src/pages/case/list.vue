...
const hint = computed(() =>
  kind.value === 'procurement'
    ? '此处只列国内采购合同/备货。打开后填写采购合同；保存或推进前必须关联一笔已签订的销售合同。货款可选一次性付清或分期支付（分期须填约定付款时间、付款比例、金额）。'
    : '此处只列出口销售合同。打开后填写并提交销售合同（销售合同/订单确认）。国内采购订单不在本列表。',
);
...
function secondary(c: any) {
  ...
  return `${c.goodsDesc || ''} · ${node} · ${status} · ${amt}`.replace(/^ · /, '');
}
...
function badgeClass(c: any) {
  if (kind.value === 'sales') return c.signed ? 'badge-pass' : 'badge-soft';
  return decisionClass(c.status === 'BLOCKED' ? 'HARD_BLOCK' : c.overallRisk);
}

function badgeText(c: any) {
  if (kind.value === 'sales') return c.signed ? '已签订' : '待签订';

...
const hint = computed(() =>
  kind.value === 'procurement'
    ...
    : '出口销售合同按未出运、已出运、已完成分组。已完成须已出运、客户已提货且收汇对账已回款。打开卡片仍填写销售合同（销售合同/订单确认）。国内采购订单不在本列表。本案已离开销售合同节点时，可点「进入下一节点」。下一步若是装运或单证，与首页出运管理、单证管理同一路径：装运先选择或新建批次再进入该批装运；单证只选择已有批次，没有批次时提示先去出运，不会打开空白单证页。收汇仍在该批次内从单证继续。',
);
